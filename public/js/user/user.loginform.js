...
midas.user.login.loginResult = function (responseText) {
    ...
    try {
        ...
        if(resp.status && resp.redirect) {
            if($('input[name=remerberMe]').is(':checked') && window.localStorage && typeof resp.localStore != "undefined")
              {
              window.localStorage.setItem('DataStoreID', resp.localStore)
              }
            $.post(json.global.webroot+'/api/json?method=midas.slicerdatastore.authenticate', {data: resp.localStore}, function(ret){
